...
function initChatServerAdapter() {
    var receiveCallbacks = [];
     ...
     server.on('wait', function() {
	  alert("Please wait while your login details are being verfied.");
    });
	server.on('success', function() {
	  alert("Login Successful. You can now chat.");
    });
	server.on('retry', function() {
		alert("Invalid credentials. Click OK to reload the page and retry.");
		window.location.reload();
	});
	server.on('done', function(name) {
		alert(name);
		server.emit('done', name);
	});

    server.on('receive', function(message) {
        var
            msg_sender = $('<div />').text(message.sender).html(),
        ...
        for (
            ...
            receiveCallbacks[funcNo](msg_sender, msg);
    });

    return {
        setName:function(name,pass) {server.emit('setName', name, pass)},
...
function initCursorServerAdapter() {
    ...
    server.on('updateCursor', function(cursor) {
        var
            sender = $('<div />').text(cursor.name).html(),
        ...
        for (
            ...
            receiveCallbacks[funcNo](sender, position);
    });

    server.on('removeCursor', function(name) {
        var sender = $('<div />').text(name).html();
        for (
            var
                funcNo = -1,
                numFuncs = receiveCallbacks.length
            ...
            receiveCallbacks[funcNo](sender);
    });
...
function initChatModel() {
    var
        ...
        messages = [
            {
                sender:'Server',
                message:'Welcome.. :-) u can type and chat as well as draw and chat. Hold shift for st lines, circles.'
            }
        ],
        messageAddCallbacks = []
    ;
    ...
    function setName(newName, newPass) {
        name = newName;
	    pass = newPass;
        chatServer.setName(name, pass);
    }
...
function initChatController() {
    ...
    function initUi() {
	   ...
	   passInput.keydown(function(event) {
            if (event.which == 13)
                signIn()
        });
